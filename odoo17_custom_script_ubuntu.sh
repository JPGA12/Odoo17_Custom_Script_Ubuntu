...
rm -rf /opt/odoo17/odoo/extra-addons
rm -rf /opt/odoo17/odoo/enterprise

# Crear directorios limpios
mkdir -p /opt/odoo17/odoo/extra-addons
mkdir -p /opt/odoo17/odoo/enterprise

# Clonar repositorios originales (sin tokens privados)
...
# Recargar servicios y reiniciar Odoo
...
sudo systemctl stop odoo17-server
sudo systemctl stop odoo17
sudo pkill -f odoo

# Remove old service
sudo systemctl stop odoo17-server
sudo systemctl disable odoo17-server
sudo rm /etc/systemd/system/odoo17-server.service

# Reload systemd and restart Odoo
sudo systemctl daemon-reload
